...
class TextClassificationModel:
    def __init__(self, model_name='openai-community/gpt2', cache_dir=None):
        """
        Initializes the GPT-2 model, tokenizer, and device.
        :param model_name: Name of the pre-trained GPT-2 model (default: openai-community/gpt2)
        ...
        self.tokenizer = GPT2Tokenizer.from_pretrained(model_name, cache_dir=self.cache_dir)
        self.tokenizer.pad_token = self.tokenizer.eos_token  # GPT-2 doesn't have padding token by default
    ...
    def tokenize_data(self, texts):
        """Tokenizes the text data using the GPT-2 tokenizer."""
        return self.tokenizer(texts, truncation=True, padding=True, max_length=512, return_tensors="pt")
    ...
    def compute_metrics(self, p):
        ...
        return {
            'accuracy': acc,
            'precision_per_class': precision.tolist(),  # Convert to list
            'recall_per_class': recall.tolist(),        # Convert to list
            'f1_per_class': f1.tolist(),                # Convert to list
            'support_per_class': support.tolist(),      # Convert to list
            'eval_precision': np.mean(precision),  # Log scalar values to TensorBoard
    ...
    def train(self, text_train, y_train, text_test, y_test, save_model_path='./saved_model'):
        """Trains the GPT-2 model using the given training and testing data."""
        ...
        model = GPT2ForSequenceClassification.from_pretrained(self.model_name, num_labels=len(set(y_train)), cache_dir=self.cache_dir).to(self.device)
        # Set the pad_token_id to eos_token_id for GPT-2
    ...
    def load_and_evaluate(self, save_model_path, text_test, y_test):
        ...
        model = GPT2ForSequenceClassification.from_pretrained(save_model_path, num_labels=len(set(y_test))).to(self.device)
    ...
    def predict(self, text_test, y_test, save_predictions_path, save_model_path):
        ...
        """
        # Load fine-tuned GPT-2 model (corrected from BERT to GPT-2)
        model = GPT2ForSequenceClassification.from_pretrained(save_model_path, num_labels=len(self.class_mapping)).to(self.device)
    ...
    def print_class_wise_metrics(self, eval_results):
        ...
        """
        # Access the per-class metrics directly
        ...
        # Assuming self.class_mapping is used to store class label names
